...
氷晶内を通る光線の完全な3D追跡、Airy／Lorenz–Mieによる波動光学、絶対強度・偏光、太陽円盤の厳密な面積分、Monte Carlo光線追跡、雨滴の非球形状は今後の増築対象です。未計算の光路や現象を計算済みとは表示しません。
...
- 既定カメラは観察者の光学原点に置き、反太陽方位へ約90°の水平視野を確保します。遠景帯と代表雨滴層は同じ観察者中心の虹角を共有します。
...
虹は距離が一意に決まる物体ではないため、300 m先に固定した虹へ実寸カメラで接近する実装にはしていません。遠景では未分解の大量雨滴が作る連続相対放射輝度を表示し、接近時に80–300 mの代表雨域へ固定した60,000滴を徐々に解像します。その中から現在の観察者中心の波長別円錐帯へ入るIDを選択でき、ズーム対象は必ずこの固定配列に存在する滴です。別の説明用水滴へ切り替えません。
...
3. クリック／タップ、重なり候補、ID指定、前後ボタンのいずれかで選んだ同じ実在ID
4. 選択滴の0.45–1.00 mm等価球模型
5. 寄与滴では目へ届く停留光線、非寄与滴では目を外れる比較光線
6. 代表7波長と部分透過枝
