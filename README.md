...
物理的な画角ではなく、学習段階を表す意味スケール指標を約300 m相当から350 µm相当まで対数補間します。大量の点表示、同じ位置に重ねた選択水滴の局所模型、内部光路をクロスフェードし、ID・世界座標・固定観察者への方向を保ったままカメラ注視点と表示倍率を連続補間します。kmとmmを同じThree.js座標の実寸として扱わない「意味ズーム」であり、画面寸法から校正した実測縮尺ではありません。
